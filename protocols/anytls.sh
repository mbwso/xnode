...
protocol_add_anytls() {
  ...
  port="$(prompt_input "端口" "${XNODE_DEFAULT_PORTS[anytls]}")"
  ...
  inbound_file="$(singbox_save_inbound "${tag}" "$(
    jq \
      ...
      ' "${XNODE_TEMPLATES}/anytls.json"
  )")"

  register_protocol "$(jq -n \
    --arg type "anytls" --arg tag "${tag}" --argjson port "${port}" \
  ...
  singbox_merge_config && singbox_check_config && firewall_allow_protocol anytls "${port}" && singbox_restart
  print_client_info anytls "${domain}" "${port}" "${password}"
  success "AnyTLS 节点已添加"
}
